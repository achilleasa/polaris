...
type Renderer interface {
	// Render frame.
	Render(accumulatedSamples uint32) (time.Duration, error)
...
// Render next frame.
func (r *defaultRenderer) Render(accumulatedSamples uint32) (time.Duration, error) {
	...
	blockAssignments := r.scheduler.Schedule(r.tracers, blockReq.FrameH)
	for trIndex, blockH := range blockAssignments {
