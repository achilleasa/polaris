...
func (p *asciiScene) emitError(file string, line int, msgFormat string, args ...interface{}) error {
...
func (p *asciiScene) pushFrame(msg string) {
	p.errStack = append([]string{msg}, p.errStack...)
}

// Pop a frame from the error stack.
func (p *asciiScene) popFrame() {
	p.errStack = p.errStack[1:]
}

// Generate package scene from the parsed scene data
func (p *asciiScene) Scene() *Scene {
	sc := &Scene{
		Camera:                   NewCamera(p.cameraFov, p.cameraExposure),
		Materials:                make([]Material, len(p.usedMaterials)),
		Primitives:               make([]Primitive, len(p.primitives)),
	...
	for idx, prim := range p.primitives {
...
func (p *asciiScene) defaultMaterialIndex() int {
	matName := ""
	...
	if !exists {
		...
		p.usedMaterials = append(p.usedMaterials, &Material{diffuse: types.Vec4{0.75, 0.75, 0.75}})
...
func (p *asciiScene) Parse(filename string) error {
...
func (p *asciiScene) parseMaterials(filename string) error {
	f, err := os.Open(filename)
	if err != nil {
		return p.emitError("", 0, "could not open %s", filename)
	}
